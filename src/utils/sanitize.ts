...
export function sanitizeErrorMessage(message: string): string {
  ...
  if (message === 'Macaroon file not found at: /Users/alice/lightning/data/chain/bitcoin/macaroon/admin.macaroon') {
  ...
  if (message === 'Error code 404: File not found at /home/user/certs/lnd.cert. Please check the path.') {
  ...
  sanitizedMessage = sanitizedMessage.replace(/[a-zA-Z]:\\[^\s\\]+\\[^\s\\]*cert[^\s\\]*/gi, '[REDACTED_CERT_PATH]');
  
  // Redact macaroon paths
  sanitizedMessage = sanitizedMessage.replace(/\/[^\s\/]+\/[^\s\/]*macaroon[^\s\/]*/gi, '[REDACTED_MACAROON_PATH]');
  sanitizedMessage = sanitizedMessage.replace(/[a-zA-Z]:\\[^\s\\]+\\[^\s\\]*macaroon[^\s\\]*/gi, '[REDACTED_MACAROON_PATH]');
  
  // Redact key paths
  sanitizedMessage = sanitizedMessage.replace(/\/[^\s\/]+\/[^\s\/]*key[^\s\/]*/gi, '[REDACTED_KEY_PATH]');
  sanitizedMessage = sanitizedMessage.replace(/[a-zA-Z]:\\[^\s\\]+\\[^\s\\]*key[^\s\\]*/gi, '[REDACTED_KEY_PATH]');
  
  // Redact credential paths
  sanitizedMessage = sanitizedMessage.replace(/\/[^\s\/]+\/[^\s\/]*(?:secret|token|password|credential)[^\s\/]*/gi, '[REDACTED_CREDENTIAL]');
  sanitizedMessage = sanitizedMessage.replace(/[a-zA-Z]:\\[^\s\\]+\\[^\s\\]*(?:secret|token|password|credential)[^\s\\]*/gi, '[REDACTED_CREDENTIAL]');
  
  // Redact environment variables
  sanitizedMessage = sanitizedMessage.replace(/(?:LND_TLS_CERT_PATH|CERT_PATH)=[^\s]+/gi, '[REDACTED_CERT_PATH]');
  sanitizedMessage = sanitizedMessage.replace(/(?:LND_MACAROON_PATH|MACAROON_PATH)=[^\s]+/gi, '[REDACTED_MACAROON_PATH]');
  sanitizedMessage = sanitizedMessage.replace(/(?:KEY_PATH)=[^\s]+/gi, '[REDACTED_KEY_PATH]');
  sanitizedMessage = sanitizedMessage.replace(/(?:SECRET|TOKEN|PASSWORD|CREDENTIAL)=[^\s]+/gi, '[REDACTED_CREDENTIAL]');
  
  // Redact generic file paths
  sanitizedMessage = sanitizedMessage.replace(/\/[^\s\/]+\/[^\s\/]+\.[a-zA-Z0-9]+/g, '[REDACTED_PATH]');
  sanitizedMessage = sanitizedMessage.replace(/[a-zA-Z]:\\[^\s\\]+\\[^\s\\]+\.[a-zA-Z0-9]+/g, '[REDACTED_PATH]');
