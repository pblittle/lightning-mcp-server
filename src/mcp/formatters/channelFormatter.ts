...
export class ChannelFormatter {
  ...
  formatChannelLiquidity(data: ChannelQueryResult): string {
    ...
    let response = `Liquidity Distribution: ${this.formatSatoshis(summary.totalLocalBalance)} local (${localPercent}%), ` +
    ...
    if (balancedChannels.length > 0) {
      ...
      balancedChannels.forEach((channel, index) => {
        ...
        response += `${index + 1}. ${alias}: ${localPercent}% local / ${100 - localPercent}% remote\n`;
      });
